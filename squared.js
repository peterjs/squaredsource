...
//    baconEventStream.onValue(function() {alert('hello frp!')});
    ...
    function cal(user, path) {
        console.log(path + ' ' + user);
        return path + ' ' + user;
    }

//    var c = user.toProperty('').combine(repo.toProperty(''),cal);
//    c.log();

    function f(user, repo) {
        console.log(repo + ' ' + user);
        makeCalendar(repo, user);
    }
//    Bacon.onValues(user, repo, f);

    var validFoldersRepos = repo.flatMapLatest(function(path){
       return Bacon.fromCallback(fs.exists, path);
    });

    validFoldersRepos.log();

    function toLogsStream(path) {
        return Bacon.fromNodeCallback(gw.log, path);
    }

//    var logsStream = repo.flatMapLatest(toLogsStream);
//    logsStream.log();
};